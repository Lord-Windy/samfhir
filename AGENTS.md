...
```bash
st add --project SAMFHIR --summary "Implement auth middleware" \
...
```bash
echo '{"project":"SAMFHIR","summary":"Implement auth middleware","description":"Add JWT validation to all API routes","labels":["backend","security"]}' | st add --json
```
...
cat <<'EOF' | st add --json
[
  {"project":"SAMFHIR","summary":"Design database schema","labels":["backend"]},
  {"project":"SAMFHIR","summary":"Build API endpoints","labels":["backend"]},
  {"project":"SAMFHIR","summary":"Create frontend components","labels":["frontend"]}
]
EOF
...
st add --project SAMFHIR --summary "Design database schema"
# Output: Created task: a1b2c3d4

# Create a dependent ticket
st add --project SAMFHIR --summary "Build API endpoints" --depends-on a1b2c3d4
```
...
cat <<'EOF' | st add --json
[
  {"project":"SAMFHIR","summary":"Design database schema"},
  {"project":"SAMFHIR","summary":"Build API endpoints","dependencies":["a1b2c3d4"]}
]
EOF
```
...
```bash
st list --ready --project SAMFHIR --label backend
```
...
| Action | Command |
|---|---|
| Create a ticket | `st add --project SAMFHIR --summary "..."` |
| Create with dependency | `st add --project SAMFHIR --summary "..." --depends-on KEY` |
| List ready tickets | `st list --ready --project SAMFHIR [--json]` |
